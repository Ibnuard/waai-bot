...
io.on("connection", (socket) => {
  ...
  socket.on("start-wa", () => {
    console.log(
      `[EVENT] Menerima permintaan 'start-wa' dari client: ${socket.id}`,
    );
    waManager.init();
  });
  ...
  socket.on("ai-test", async (config) => {
    waManager.addLog(
      `Mengetes koneksi AI ke ${config.baseUrl || config.provider}...`,
      "info",
    );
  ...
  socket.on("ai-gemini-models-fetch", async (apiKey) => {
    try {
      ...
      socket.emit("ai-gemini-models-data", {
        success: false,
  ...
  socket.on("ai-local-models-fetch", async (baseUrl) => {
    try {
      ...
      socket.emit("ai-local-models-data", {
        success: false,
        message: error.message,
      });
    }
