    ...
    try {
      ...
      this.sock.ev.on("connection.update", (update) => {
        ...
        if (connection === "close") {
          ...
          this.addLog(
            `Koneksi terputus: ${lastDisconnect.error?.message || "Reason unknown"} (Code: ${statusCode})`,
            "error",
          );

          if (shouldReconnect) {
            this.addLog(
              "Mencoba menyambung kembali otomatis dalam 5 detik...",
              "info",
            );
          ...
          if (process.env.APP_MODE === "trial") {
            this.addLog(
              "TRIAL MODE: WhatsApp akan terputus otomatis dalam 5 menit.",
              "warning",
            );
            ...
            this.trialTimeout = setTimeout(
              async () => {
                this.addLog(
                  "TRIAL MODE EXPIRED: Memutuskan koneksi...",
                  "error",
                );
                await this.stop();
              },
      ...
      this.sock.ev.on("messages.upsert", async (m) => {
        ...
        this.addLog(
          `[${pushName}] (${this.normalizeJid(from)}): ${content.substring(0, 50)}${content.length > 50 ? "..." : ""}`,
          "message",
        );

        const profile = configManager
          .getAiConfig()
          .profiles.find(
            (p) => p.id === configManager.getAiConfig().activeProfileId,
          );
        ...
        console.log(
          `[DEBUG] Check Trigger - msg: "${cleanContent}", trigger: "${trigger}", profile: "${profile.name}", enabled: ${profile.enabled}, respondAll: ${profile.respondAll}, allowGroups: ${profile.allowGroups}`,
        );
        ...
        let shouldRespond =
          profile.enabled &&
          (profile.respondAll ||
            cleanContent.toLowerCase().startsWith(trigger));
        ...
        if (shouldRespond) {
          ...
          try {
            this.addLog(
              `[AI] Memproses pesan via ${profile.name} (${profile.provider})...`,
              "info",
            );
